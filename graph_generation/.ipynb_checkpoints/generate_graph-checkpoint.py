...
def generate_forest_fire_graph(n, p, r, prob_selected = 0.3, seed=None):
    """
    Generate a (un)directed Forest Fire graph with n nodes using parameters:
        p  - forward burning probability
        r  - backward burning ratio

    The geometric variables follow the specification in the original model:
        x ~ geometric(mean = p/(1-p))
        y ~ geometric(mean = r*p/(1 - r*p))

    Returns a NetworkX DiGraph where the graph is assumed to be undirected as all edges are added in reverse at the end.
    """
    if seed is not None:
        random.seed(seed)

    G = nx.DiGraph()
    G.add_node(0)  # first node

    # Geometric distribution helper: P(X=k) = (1-q)^k * q  for k >= 0
    def geometric_sample(q):
        # geometric mean = (1-q)/q, so q = 1 / (mean + 1)
        # but we already receive q directly
        count = 0
        while random.random() > q:
            count += 1
        return count

    # Convert means to geometric q parameters
    # mean = (1-q)/q  =>  q = 1/(mean+1)
    mean_x = p / (1 - p)
    q_x = 1 / (mean_x + 1)
    ...
    for v in range(1, n):
        ...
        # Step (i): pick ambassador
        ambassador = random.choice(list(G.nodes)[:-1])
        visited = set([ambassador])

        # Connect to the ambassador
        G.add_edge(v, ambassador)

        # recursive burning queue
        queue = [ambassador]

        while queue:
            w = queue.pop()
            # neighbors not yet visited
            out_neighbors = [u for u in G.successors(w) if u not in visited]
            in_neighbors = [u for u in G.predecessors(w) if u not in visited]

            # Step (ii): sample x, y
            x = geometric_sample(q_x)
            y = geometric_sample(q_y)

            # choose available out-neighbors
            chosen_out = random.sample(out_neighbors, min(x, len(out_neighbors)))
            # choose available in-neighbors
            chosen_in = random.sample(in_neighbors, min(y, len(in_neighbors)))

            next_nodes = chosen_out + chosen_in

            # form links and recurse
            for u in next_nodes:
                if u not in visited:
                    visited.add(u)
                    G.add_edge(v, u)
            ...
            mask = torch.rand(n) < prob_selected  # Random selection based on prob_selected
